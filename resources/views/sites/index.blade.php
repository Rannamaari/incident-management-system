...
@section('header')
    <div class="flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <p class="mt-1 sm:mt-2 text-sm sm:text-base lg:text-lg text-gray-600 dark:text-gray-400 font-medium">Active and monitoring sites</p>
        </div>
...
@section('content')
    <div class="py-4 sm:py-6" x-data="{ showFilters: false }">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <!-- Search and Filters -->
            <div class="mb-4 sm:mb-6 rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-gray-700/50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg">
                <form method="GET" action="{{ route('sites.index') }}" class="p-4 sm:p-6 space-y-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" name="search" value="{{ request('search') }}"
                            ...
                            class="w-full rounded-xl sm:rounded-2xl border border-gray-300 dark:border-gray-600/50 pl-12 pr-4 py-3 sm:py-3.5 text-sm sm:text-base shadow-sm focus:border-blue-600 dark:focus:border-blue-400 focus:ring-2 focus:ring-blue-600/20 dark:focus:ring-blue-400/20 touch-manipulation">
                    </div>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                        ...
                        <select name="site_type" class="rounded-xl border border-gray-300 dark:border-gray-600/50 px-3 sm:px-4 py-2.5 sm:py-3 text-sm sm:text-base touch-manipulation">
                        ...
                        <button type="button" @click="showFilters = !showFilters"
                            class="rounded-xl border-2 border-blue-600 bg-white dark:bg-gray-800 px-4 py-2.5 sm:py-3 text-sm sm:text-base font-semibold text-blue-600 hover:bg-blue-50 transition-colors touch-manipulation flex items-center justify-center gap-2">
                    ...
                    <div x-show="showFilters"
                        ...
                        <h3 class="text-sm font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wide">Advanced Filters</h3>

                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Link Site Filter -->
                            <div class="flex items-center gap-3 p-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-blue-50/50">
                                ...
                                <label for="filter_link" class="flex-1 cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-100">
                                    Link Sites Only
                                </label>
                            </div>

                            <!-- Temp Site Filter -->
                            <div class="flex items-center gap-3 p-3 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-orange-50/50">
                                <input type="checkbox" name="is_temp_site" id="filter_temp" value="1" {{ request('is_temp_site') ? 'checked' : '' }}
                                    class="h-5 w-5 rounded border-gray-300 dark:border-gray-600 text-orange-600 focus:ring-orange-500 touch-manipulation">
                                <label for="filter_temp" class="flex-1 cursor-pointer text-sm font-medium text-gray-900 dark:text-gray-100">
                        ...
                        <!-- Technologies Filter -->
                        <div class="p-4 rounded-xl border-2 border-gray-200 dark:border-gray-700 bg-gray-50/50">
                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-3">Filter by Technologies:</label>
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                @foreach(['2G', '3G', '4G', '5G', 'ILL', 'SIP', 'IPTV', 'NCIT'] as $tech)
                                    <label class="flex items-center gap-2 p-2 rounded-lg border border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 hover:bg-blue-50 cursor-pointer transition-colors">
                                        <input type="checkbox" name="technologies[]" value="{{ $tech }}"
                                            {{ in_array($tech, request('technologies', [])) ? 'checked' : '' }}
                                            class="h-4 w-4 rounded border-gray-300 dark:border-gray-600 text-blue-600 focus:ring-blue-500 dark:focus:ring-blue-400 touch-manipulation">
                                        <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $tech }}</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                    ...
                    <div class="flex gap-3 pt-2">
                        ...
                        <a href="{{ route('sites.index') }}" class="flex-1 sm:flex-none rounded-xl border-2 border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-6 py-3 text-sm sm:text-base font-heading font-semibold text-gray-700 dark:text-gray-300 text-center hover:bg-gray-50 transition-colors touch-manipulation">
                            Clear All
                        </a>
                    </div>
                </form>
            ...
            <div class="lg:hidden space-y-4">
                    ...
                    <div class="rounded-2xl border border-gray-100 dark:border-gray-700/50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg p-4 hover:shadow-xl transition-shadow">
                        <div class="flex items-start justify-between mb-3">
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-600 dark:text-gray-400 mt-1">{{ $site->site_name ?: $site->display_name }}</p>
                            </div>
                        ...
                        <div class="space-y-2 mb-3">
                            <div class="flex items-center gap-2 text-sm">
                                ...
                                <span class="font-medium text-gray-900 dark:text-gray-100">{{ $site->region->code }}</span>
                            </div>
                ...
                @empty
                    <div class="text-center py-12 rounded-2xl border border-gray-200 dark:border-gray-700 bg-white/50">
                        ...
                        <p class="mt-4 text-base font-medium text-gray-900 dark:text-gray-100">No sites found</p>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200/50">
                        <thead class="bg-gradient-to-r from-slate-50/80 to-white/60">
                            <tr>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Site Code</th>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Region</th>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Site Name</th>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Technologies</th>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Flags</th>
                                <th class="px-4 py-4 text-left text-xs font-heading font-semibold text-gray-700 dark:text-gray-300 uppercase">Type</th>
                        ...
                        <tbody class="divide-y divide-gray-200/30 bg-white/60">
                            @forelse($sites as $site)
                                <tr class="hover:bg-blue-50/30 transition-colors">
                                    ...
                                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-gray-100">{{ $site->region->code }}</td>
                                    <td class="px-4 py-4 text-sm text-gray-900 dark:text-gray-100">{{ $site->site_name ?: $site->display_name }}</td>
