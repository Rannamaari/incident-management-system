                ...
                <form method="POST" action="{{ route('sites.store') }}"
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-3 sm:p-4">
                        <div class="flex items-center gap-2">
                            ...
                            <div>
                                <p class="text-sm font-medium text-gray-700">Site Code (Auto-generated)</p>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            ...
                            <select name="region_id"
                                    ...
                                    required
                                    class="w-full rounded-xl sm:rounded-2xl border px-4 py-3 sm:py-3.5 text-base @error('region_id') border-red-300 @else border-gray-300 @enderror touch-manipulation">
                        ...
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   value="{{ old('site_name') }}"
                                   required
                                   class="w-full rounded-xl sm:rounded-2xl border px-4 py-3 sm:py-3.5 text-base @error('site_name') border-red-300 @else border-gray-300 @enderror touch-manipulation"
                                   placeholder="e.g., Bodufulhadhoo">
                            @error('site_name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                            <p class="mt-1 text-xs text-gray-500">Enter the name of the site/island</p>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                        <div>
                            ...
                            <input type="text"
                                   ...
                                   class="w-full rounded-xl sm:rounded-2xl border border-gray-300 px-4 py-3 sm:py-3.5 text-base @error('display_name') border-red-300 @enderror touch-manipulation"
                                   placeholder="Leave empty for auto-generated">
                            @error('display_name')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                            <p class="mt-1 text-xs text-gray-500">If left empty, site code will be used</p>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">
                                Transmission / Backhaul <span class="text-gray-500">(Optional)</span>
                            </label>
                            <input type="text"
                                   name="transmission_backhaul"
                                   value="{{ old('transmission_backhaul') }}"
                                   class="w-full rounded-xl sm:rounded-2xl border border-gray-300 px-4 py-3 sm:py-3.5 text-base @error('transmission_backhaul') border-red-300 @enderror touch-manipulation"
                                   placeholder="e.g., Fiber, Microwave">
                            @error('transmission_backhaul')<p class="mt-1 text-sm text-red-600">{{ $message }}</p>@enderror
                            <p class="mt-1 text-xs text-gray-500">Backhaul technology</p>
                        </div>
                    </div>

                    <!-- Site Settings Checkboxes -->
                    <div class="border border-gray-200 rounded-xl p-4 sm:p-6">
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-4">Site Settings</h3>
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-3 sm:gap-4">
                            <label class="flex items-start gap-3 p-4 rounded-lg border-2 border-gray-200 hover:border-green-300 hover:bg-green-50 cursor-pointer transition-colors active:scale-[0.98] touch-manipulation">
                                <input type="checkbox"
                                       name="is_active"
                                       {{ old('is_active', true) ? 'checked' : '' }}
                                       class="mt-1 h-5 w-5 rounded border-gray-300 text-green-600 focus:ring-green-500 touch-manipulation">
                                <div class="flex-1">
                                    <span class="text-sm font-semibold text-gray-900">Active Site</span>
                                    <p class="text-xs text-gray-600 mt-1">Site is operational. If unchecked, will be marked as temporary</p>
                                </div>
                            </label>
                            ...
                            <label class="flex items-start gap-3 p-4 rounded-lg border-2 border-gray-200 hover:border-blue-300 hover:bg-blue-50 cursor-pointer transition-colors active:scale-[0.98] touch-manipulation">
                                <input type="checkbox"
                                       ...
                                       class="mt-1 h-5 w-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 touch-manipulation">
                                <div class="flex-1">
                                    <span class="text-sm font-semibold text-gray-900">Link Site</span>
                                    <p class="text-xs text-gray-600 mt-1">Site is used as a network link/relay point</p>
                                </div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div class="border border-gray-200 rounded-xl p-6">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <label class="flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors"
                                ...
                                <div class="flex-1">
                                    <span class="text-base font-bold text-gray-900">End Site</span>
                                    <p class="text-sm text-gray-600 mt-1">Standard site that connects to hub sites for backhaul</p>
                                </div>
                            </label>
                            ...
                            <label class="flex items-start gap-3 p-4 rounded-xl border-2 cursor-pointer transition-colors"
                                ...
                                <input type="radio"
                                       ...
                                       class="mt-1 h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500">
                                <div class="flex-1">
                                    <span class="text-base font-bold text-gray-900">Hub Site</span>
                                    <p class="text-sm text-gray-600 mt-1">Central site that provides backhaul to multiple end sites</p>
                                </div>
                            </label>
                        </div>
                    ...
                    <div x-show="showHubSitesDropdown"
                         ...
                         class="border border-gray-200 rounded-xl p-4 sm:p-6">
                        <h3 class="text-base sm:text-lg font-semibold text-gray-900 mb-2">Connected to Hub Sites</h3>
                        <p class="text-sm text-gray-600 mb-4">Select which hub sites this end site connects to for backhaul</p>
                        ...
                        @else
                            <div class="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-300">
                                ...
                                <p class="mt-2 text-sm text-gray-600">No hub sites available yet.</p>
                                <p class="text-xs text-gray-500">Create hub sites first to connect end sites to them.</p>
                            </div>
                        @endif
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Remarks <span class="text-gray-500">(Optional)</span>
                        </label>
                        <textarea name="remarks"
                                  rows="4"
                                  class="w-full rounded-xl sm:rounded-2xl border border-gray-300 px-4 py-3 sm:py-3.5 text-base @error('remarks') border-red-300 @enderror touch-manipulation"
                    ...
                    <div class="flex flex-col sm:flex-row items-stretch sm:items-center justify-end gap-3 sm:gap-4 pt-6 border-t">
                        <a href="{{ route('sites.index') }}"
                           class="order-2 sm:order-1 text-center rounded-xl border-2 border-gray-300 px-6 py-3.5 text-base font-semibold text-gray-700 hover:bg-gray-50 transition-colors touch-manipulation">
                            Cancel
                        </a>
                        <button type="submit"
