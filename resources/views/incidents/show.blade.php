            ...
            <div class="flex flex-wrap justify-between items-center gap-3 mb-6">
                ...
                <div class="flex items-center gap-2">
                    ...
                    @if(auth()->user()->canEditIncidents())
                        ...
                        <form action="{{ route('incidents.send-notification', $incident) }}" method="POST" class="inline-block">
                            @csrf
                            <button type="submit"
                                class="p-2 rounded-lg bg-indigo-100 dark:bg-indigo-900/30 hover:bg-indigo-200 dark:hover:bg-indigo-800/40 text-indigo-600 dark:text-indigo-400 transition-all duration-200"
                                title="Send Email Notification"
                                onclick="return confirm('Send email notification for this incident?');">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
