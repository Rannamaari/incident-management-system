...
@section('header')
    <!-- Hero -->
    <div class="-mx-4 sm:-mx-6 lg:-mx-8 bg-gradient-to-br from-slate-50 via-white to-blue-50/50 px-4 sm:px-6 lg:px-8 py-8 border-b border-gray-200 dark:border-gray-700/30">
                ...
                <div class="min-w-0 flex-1">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="mt-2 text-lg text-gray-600 dark:text-gray-400 font-medium">Manage system users and their access levels</p>
                        </div>
                    </div>
                </div>
...
@section('content')
    <div class="py-6 lg:py-8">
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <!-- Filters -->
            <div class="mb-8 overflow-hidden rounded-3xl border border-gray-100 dark:border-gray-700/50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg">
                <details class="group" @if(request()->hasAny(['search', 'role'])) open @endif>
                    <summary class="cursor-pointer list-none px-6 py-5 border-b border-gray-200 dark:border-gray-700/50 hover:bg-gray-50/50 transition-colors duration-200">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <div class="grid h-8 w-8 place-items-center rounded-xl bg-gradient-to-br from-gray-100 to-gray-200">
                                    <svg class="h-4 w-4 text-gray-600 dark:text-gray-400" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M3 4a1 1 0 011-1h16a1 1 0 011 1v2.586a1 1 0 01-.293.707l-6.414 6.414a1 1 0 00-.293.707V17l-4 4v-6.586a1 1 0 00-.293-.707L3.293 7.207A1 1 0 013 6.5V4z" />
                                    </svg>
                                </div>
                                <h3 class="font-heading text-lg font-heading font-semibold text-gray-900 dark:text-gray-100">Search & Filter</h3>
                                @if(request('search') || request('role'))
                                    <span class="rounded-full bg-blue-100 px-2.5 py-0.5 text-xs font-heading font-medium text-blue-800">Active</span>
                                @endif
                            </div>
                            <svg class="h-5 w-5 text-gray-500 dark:text-gray-400 transition-transform duration-200 group-open:rotate-180" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <form method="GET" action="{{ route('users.index') }}" class="p-6 space-y-4">
                        <div class="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                            <div>
                                <label for="search" class="block text-sm font-heading font-medium text-gray-700 dark:text-gray-300 mb-2">Search</label>
                                <input type="text" name="search" id="search" value="{{ request('search') }}"
                                    placeholder="Search by name or email..."
                                    class="w-full rounded-xl border border-gray-300 dark:border-gray-600 px-4 py-2 text-sm focus:border-blue-500 focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-400/20 transition-all duration-200">
                            </div>
                            <div>
                                <label for="role" class="block text-sm font-heading font-medium text-gray-700 dark:text-gray-300 mb-2">Role</label>
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            @if(request()->hasAny(['search', 'role']))
                                <a href="{{ route('users.index') }}"
                                    class="inline-flex items-center gap-2 rounded-xl border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-6 py-2.5 font-semibold text-gray-700 dark:text-gray-300 shadow-sm transition-all duration-300 hover:bg-gray-50">
                                    Clear
                                </a>
                            @endif
                        </div>
            ...
            <!-- Users Table -->
            <div class="overflow-hidden rounded-3xl border border-gray-100 dark:border-gray-700/50 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm shadow-lg">
                                ...
                                <tr onclick="window.location='{{ route('users.edit', $user) }}'"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                            ...
                                            <div class="ml-4">
                                                <div class="text-sm font-heading font-medium text-gray-900 dark:text-gray-100">{{ $user->name }}</div>
                                                @if($user->id === auth()->id())
                                                    <div class="text-xs text-gray-500 dark:text-gray-400">(You)</div>
                                                @endif
                                            </div>
                                        </div>
                                    </td>
                                ...
                                <tr>
                                    <td colspan="5" class="px-6 py-12 text-center">
                                        <div class="flex flex-col items-center justify-center">
                                            ...
                                            <p class="text-sm font-heading font-medium text-gray-900 dark:text-gray-100">No users found</p>
